...
export function resolveExternalCliRunnerStatus(input: {
	...
	return {
		...
		...(codexExecWriter ? { safety: { access: "workspace-write" as const, sandbox: "workspace-write" as const, approvalPolicy: "never" as const, ephemeral: true as const, publication: "blocked-by-sandbox" as const } } : {}),
		...
		...(claudeCodeWriter ? { safety: { access: "workspace-write" as const, authentication: "existing-cli-required" as const, permissionMode: "acceptEdits" as const, tools: "Read,Write,Edit,Glob,Grep" as const, mcp: "empty-strict" as const, settingSources: "user" as const, userSettingsTrust: "required" as const, sessionPersistence: false as const, publication: "blocked-by-tool-allowlist" as const } } : {}),
		...
		...(cursorAgentWriter ? { safety: { access: "workspace-write" as const, authentication: "cursor-api-key-or-existing-login" as const, mode: "print" as const, sandbox: "enabled" as const, workspaceTrust: "existing-required" as const, sessionReuse: false as const, publication: "blocked-by-sandbox" as const } } : {}),
		capabilities: {
			stop: true,
			steer: false,
			resume: false,
