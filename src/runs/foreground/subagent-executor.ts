...
import { discoverAgents, findBlockingAgentDiagnostic, formatUnknownAgentError, resolveAgentName, unknownAgentDiagnosticContext } from "../../agents/agents.ts";
import type { AgentConfig, AgentDiscoveryDiagnostic } from "../../agents/agent-contract.ts";
import type { AgentScope, UnknownAgentDiagnosticContext } from "../../shared/core-contracts.ts";
...
import { claimRunFanoutBatch, createRunFanoutBudget, decodeRunFanoutBudgetDescriptor, formatRunFanoutBudget, getRunFanoutBudgetSnapshot, RunFanoutLimitError, RUN_FANOUT_BUDGET_ENV, writeRunFanoutBudgetDescriptor } from "../shared/run-fanout-budget.ts";
...
import { waitForImportedAsyncRoot } from "../background/chain-root-attachment.ts";
...
import { createWorkflowState, removeSettledWorkflowState } from "../../workflows/workflow-state.ts";
...
type DirectExecutionParams = Omit<SubagentParamsLike, "chain" | "tasks" | "parallel" | "concurrency" | "chainDir"> & {
	agent?: string;
	chain?: never;
	tasks?: never;
	parallel?: never;
...
function canonicalizeExecutionParams(params: SubagentParamsLike, agents: AgentConfig[], diagnostics: AgentDiscoveryDiagnostic[] | undefined, context: UnknownAgentDiagnosticContext): { params?: SubagentParamsLike; error?: string } {
	...
	const result = canonicalizeAgentName(params.agent, agents, diagnostics, context);
...
function validateExecutionInput(
	...
	if (!params.agent) {
		return {
			content: [{ type: "text", text: `Provide an agent. Agents: ${agents.map((agent) => agent.name).join(", ") || "none"}` }],
			isError: true,
	...
	if (acceptanceErrors.length > 0) {
		return { content: [{ type: "text", text: acceptanceErrors.join(" ") }], isError: true, details: { mode: "single", results: [] } };
	}
	if (!agents.some((agent) => agent.name === params.agent)) {
		return { content: [{ type: "text", text: formatUnknownAgentError(params.agent, context) }], isError: true, details: { mode: "single", results: [] } };
	}
	return null;
}
...
function collectStaticLaunchSummaries(input: {
	...
	if (!input.params.agent) return [];
	return [resolveStaticLaunchSummary({
		agent: input.params.agent,
		index: 0,
		explicitModel: input.params.model,
		agents: input.agents,
		parentModel: input.parentModel,
...
async function preflightForkSession(
	params: SubagentParamsLike,
	contextPolicy: AgentDefaultContextPolicy,
	prepareSessionForTask: PrepareForkSessionForTask,
): Promise<void> {
	if (contextPolicy.usesFork && params.agent && shouldForkAgent(contextPolicy, params.agent)) {
		await prepareSessionForTask(params.agent, 0, params.model);
	}
}
...
function prepareWorkflowChildLaunchParams(input: {
	...
	options?: { suppressRoutineResultIntercom?: boolean; awaitDetachedChild?: boolean; runFanoutBudget?: RunFanoutBudgetDescriptor; parentDeadlineAt?: number; capabilityCeiling?: ResolvedSubagentCapabilityCeiling; parallel?: boolean };
}): SubagentParamsLike {
...
export function prepareWorkflowLaunchParams(
	...
	parentWorkflowRunId: string,
	workflowKey: string,
	options: { suppressRoutineResultIntercom?: boolean; awaitDetachedChild?: boolean; runFanoutBudget?: RunFanoutBudgetDescriptor; parentDeadlineAt?: number; externalAsyncRequired?: boolean; capabilityCeiling?: ResolvedSubagentCapabilityCeiling; outputClaimPath?: string; parallel?: boolean } = {},
): SubagentParamsLike {
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		if (normalizedAction?.toLowerCase() === "append-step") {
			return buildRequestedModeError(requestParams, "Legacy append-step execution was removed; existing append-request artifacts are inert and remain untouched.");
		}
		if (requestParams.tasks !== undefined || requestParams.chain !== undefined || requestParams.parallel !== undefined || requestParams.concurrency !== undefined || requestParams.chainDir !== undefined) {
			return buildRequestedModeError(requestParams, "Legacy top-level chain and parallel execution was removed; use workflowScript.");
		}
		...
		if (requestParams.workflowScript !== undefined && normalizedAction === undefined) {
			...
			const foregroundWorkflowStatePath = workflowRunId
				? undefined
				: path.join(workflowArtifactsDir, `.workflow-state-${randomUUID()}.json`);
			const workflowStatePath = workflowRunId
				? path.join(DIRS.async, workflowRunId, "workflow-state.json")
				: foregroundWorkflowStatePath!;
			const workflowState = createWorkflowState(workflowStatePath);
			const cleanupForegroundWorkflowState = () => {
				if (!workflowArtifactConfig.enabled && foregroundWorkflowStatePath) fs.rmSync(foregroundWorkflowStatePath, { force: true });
			};
			if (workflowRunId) {
					...
					try {
						const workflow = await runWorkflowScript({
							...
							launch: async (key, childParams, workflowSignal, admission) => {
								...
								const preparedLaunch = prepareWorkflowChildLaunchParams({
									workflowDefaults: workflowChildDefaults,
									childParams,
									parentWorkflowRunId: workflowRunId,
									workflowKey: key,
									ctxCwd: parentCwd,
									...
									configuredOutputBaseDir,
									discoverAgents: discoverWorkflowAgents,
									agents: workflowAgents,
									workflowAgentScope: workflowChildDefaults.agentScope,
									outputOverride: childOutputOverrides.get(key),
									outputClaimPath: childOutputClaimPaths.get(key),
									options: {
										awaitDetachedChild: true,
										runFanoutBudget: workflowFanoutBudget,
										parentDeadlineAt: workflowDeadlineAt,
										capabilityCeiling: workflowCapabilityCeiling,
										parallel: admission.batch,
									},
								});
								const childRequest = { ...preparedLaunch, runFanoutAdmitted: admission.admitted };
						...
						if (terminalSettlementPersisted) removeSettledWorkflowState(workflowState.path, status.state);
						persistClosed = true;
				...
				const workflow = await runWorkflowScript({
					...
					launch: async (key, childParams, workflowSignal, admission) => {
						...
						const preparedLaunch = prepareWorkflowChildLaunchParams({
							...
							outputClaimPath: childOutputClaimPaths.get(key),
							options: {
								suppressRoutineResultIntercom: chatProgress.mode === "live-card",
								runFanoutBudget: workflowFanoutBudget,
								parentDeadlineAt: workflowDeadlineAt,
								capabilityCeiling: workflowCapabilityCeiling,
								parallel: admission.batch,
		...
		const directRequestParams = paramsWithResolvedCwd as DirectExecutionParams;
		let effectiveParams: SubagentParamsLike = applyForceTopLevelAsyncOverride(
			directRequestParams,
			depth,
		...
		const validationError = validateExecutionInput(effectiveParams, agents, unknownAgentDiagnosticContext);
		if (validationError) return validationError;

		const foregroundMode = "single" as const;
		const requestedSpawns = 1;
		...
		const selectedAgentNames = [effectiveParams.agent!];
		...
		try {
			...
			if (!effectiveParams.runFanoutAdmitted) claimRunFanoutBatch(runFanoutBudget, ["single"]);
		} catch (error) {
			activeAsyncCapacity?.rollback();
		...
		try {
			if (!(effectiveParams.clarify === true && ctx.hasUI) || deps.config.forkContext?.mode === "pruned") {
				await preflightForkSession(effectiveParams, contextPolicy, prepareForkSessionForTask);
			}
		} catch (error) {
			activeAsyncCapacity?.rollback();
			return toExecutionErrorResult(effectiveParams, error, contextPolicy.contextSummary);
		...
		if (reservation.error) {
			activeAsyncCapacity?.rollback();
			return spawnBudgetErrorResult(reservation.error, foregroundMode);
		}
		...
		try {
			...
			if (workflowLaunchObserver) {
				const singleSessionName = deriveChildSessionName({ agent: effectiveParams.agent!, task: effectiveParams.task });
				workflowLaunchObservers.delete(params);
				workflowLaunchObserver({ agent: effectiveParams.agent!, ...(singleSessionName ? { sessionName: singleSessionName } : {}), sessionFile: childSessionFileForTask(effectiveParams.agent!, 0, effectiveParams.model), async: effectiveAsync, runId: effectiveAsync ? asyncRunId : runId });
			}
			const asyncResult = await runAsyncPath(execData, deps);
			if (asyncResult) {
				asyncLaunchFailed = asyncResult.isError === true;
				return withRunFanoutBudget(withResolvedContext(withForkThinkingNotes(asyncResult, forkThinkingDowngrades), contextPolicy.contextSummary), runFanoutBudget);
			}
			if (foregroundControl) {
			...
			return withRunFanoutBudget(withResolvedContext(withForkThinkingNotes(result, forkThinkingDowngrades), contextPolicy.contextSummary), runFanoutBudget, { annotateContent: runFanoutAnnotateContent });
		} catch (error) {
