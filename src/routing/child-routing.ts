...
function abortReason(signal: AbortSignal): Error {
	return signal.reason instanceof Error ? signal.reason : new Error(typeof signal.reason === "string" ? signal.reason : "Child routing request aborted.");
}
...
export async function classifyChildProfile(ctx: ExtensionContext, config: ChildRoutingConfig, request: ChildRoutingRequest, signal?: AbortSignal): Promise<ChildRoutingSelection | null> {
	...
	const abort = () => controller.abort(signal?.reason ?? new Error("Child routing request aborted."));
	if (signal?.aborted) abort(); else signal?.addEventListener("abort", abort, { once: true });
	const timer = setTimeout(() => controller.abort(new Error(`Child routing classifier timed out after ${config.classifier.timeoutMs}ms.`)), config.classifier.timeoutMs);
	timer.unref?.();
	...
	try {
		...
		const provider = slash > 0 ? resolvedClassifier.slice(0, slash) : undefined;
		const modelId = slash > 0 ? splitKnownThinkingSuffix(resolvedClassifier.slice(slash + 1)).baseModel : undefined;
		...
		const auth = await raceWithAbort(ctx.modelRegistry.getApiKeyAndHeaders(model), controller.signal);
		if (!auth.ok) throw new Error(`Child routing classifier auth failed: ${auth.error}`);
		const registered = (ctx.modelRegistry as { getRegisteredProviderConfig?: (providerName: string) => { api?: string; streamSimple?: StreamFn } | undefined }).getRegisteredProviderConfig?.(model.provider);
