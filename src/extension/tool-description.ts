...
const EXTERNAL_CLI_RUNNER_GUIDANCE = "External CLI agents use their own runner contract and do not support native Pi child options such as model override, structured output, acceptance/agent contract, tool budget, fast mode, fork context, skills, or native Pi tools unless the runner explicitly implements them.";
...
• When passing an explicit model to a child (on the call or a runs.run/runs.all item), first call { action: "models" } and copy an exact provider/id; bare ids resolve only when unique in the registry, and agent names (e.g. gpt-pro, advisor) are not model ids. Set per-run thinking with a suffix on the model string (e.g. provider/id:high; off/minimal/low/medium/high/xhigh/max); the suffix wins over the agent's thinking default.
...
• Model actions are exactly list, get, models, children.list, guide, validate, worktree.discard, lane.status, status, debug.run, interrupt, resume, steer, stop, and doctor. ${WORKFLOW_VALIDATE_GUIDANCE} ${HUMAN_ADMINISTRATION_GUIDANCE}
...
• Passing an explicit model? Call {action:"models"} first and copy an exact provider/id; bare ids resolve only when unique in the registry; agent names (e.g. gpt-pro, advisor) are not model ids. Per-run thinking is a suffix on the model string (provider/id:high; off/minimal/low/medium/high/xhigh/max), and the suffix wins over the agent's thinking default.
