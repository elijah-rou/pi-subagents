...
const HUMAN_ADMINISTRATION_GUIDANCE = "Supported human administration uses /subagents for agent authoring, /subagents-refine, /subagents-watchdog, the profile slash commands, and /subagents-fleet. Trusted RPC retains only passive legacy schedule list/show/history readers for one release. Missions, schedule execution, lane merge/supersession, broad cleanup, and panes have no supported replacement pending Package 3.";
...
• Model actions are exactly list, get, models, children.list, guide, validate, worktree.discard, lane.status, status, debug.run, interrupt, resume, steer, stop, and doctor. validate checks workflowScript or workflowScriptPath without launching children. ${HUMAN_ADMINISTRATION_GUIDANCE}
