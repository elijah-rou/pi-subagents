...
- **Respect the fixed authority policy.** Model-facing worktree discard remains confirmation-controlled; stop and steer retain their ownership checks. Use `worktree.discard` only with the durable `handoffPath`; it refuses safely without an interactive UI. Broad cleanup, spawn-budget grants, and schedule creation are not model actions in Package 1.
...
| Child commissioning or role-to-role evidence handoff | [`references/commissioning.md`](commissioning.md) |
| Role choice, review/research/cleanup techniques, or model tiering | [`references/prompting-and-roles.md`](prompting-and-roles.md) |
...
- **Setup, discovery, or intercom confusion:** run the `subagent({ action: "doctor" })` doctor action.
...
- **Child fails before starting:** inspect `subagent({ action: "status", id: "..." })`, artifact metadata, output logs, and the doctor action; loader errors usually appear in child logs.
