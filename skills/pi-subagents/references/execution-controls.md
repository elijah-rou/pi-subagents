...
Use `mission.update` while work runs to record decisions, bounded journal entries, artifacts, labels, summaries, or delivery receipts. Journal entries use `kind`, `title`, optional `body`, `evidence`, and `runId`; the store generates ids and timestamps, locks concurrent mission updates, and rejects the complete journal above its 256-entry or 256 KiB bounds. Use journal entries for compact durable decision/evidence history, not model-authored instructions. A receipt records a pull request, CI, deployment, or release link with a concise status; it does not authorize or automate merge, CI polling, or deployment. Record open product, architecture, or safety decisions there and escalate them upward; do not let a child decide silently. Use `mission.attach-run` only for runs launched outside the normal mission-backed path, and use `mission.close` with a terminal status and concise summary when the mission is done.
...
- **Use journals, artifacts, and receipts as evidence.** Add concise `mission.update` journal entries for durable decisions, hypotheses, observations, experiments, results, and corrections. Mission-backed launches already record run artifacts such as async `status.json`, `events.jsonl`, child output paths, and handoff manifests, so add artifacts only for extra durable outputs such as `patch`, `review`, or `note` files. Add receipts for external outcomes: `pull_request`, `ci`, `deployment`, or `release`; each receipt needs an absolute URL. Receipts are evidence, not authority to merge, deploy, or release.
