...
Additional user prompt templates can delegate into `pi-subagents` through the native `/prompt-workflow` command. This is useful when a slash command should always run through a particular agent or with forked context. Prompt frontmatter can set `subagent`, `model`, `skill`, `cwd`, `fresh`, `fork`, or `inheritContext` for the native adapter. Set `parent-only: true` when a prompt itself orchestrates subagents; Pi can invoke that prompt directly, while `/prompt-workflow` rejects it instead of nesting the orchestrator inside a child.
