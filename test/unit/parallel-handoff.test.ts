...
describe("parallel handoff", () => {
	...
	it("passively parses and preserves historical merge and supersession fields", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-parallel-handoff-historical-evidence-"));
		try {
			const manifestPath = path.join(dir, "handoff.json");
			writeParallelHandoffGroup({
				manifestPath,
			...
			const raw = JSON.parse(fs.readFileSync(manifestPath, "utf-8")) as ParallelHandoffManifest;
			const merge = {
				prNumber: 1623,
				...
				attestedBy: "historical-operator",
				attestedAt: "2026-08-27T16:23:00.000Z",
				manifestDigest: "0".repeat(64),
			};
			const supersession = {
				supersededBy: "replacement-lane",
				attestedBy: "historical-operator",
				attestedAt: "2026-08-27T16:24:00.000Z",
				manifestDigest: "0".repeat(64),
			};
			raw.merge = merge;
			raw.supersession = supersession;
			fs.writeFileSync(manifestPath, JSON.stringify(raw), "utf-8");

			assert.deepEqual(readParallelHandoffManifest(manifestPath).merge, merge);
			assert.deepEqual(readParallelHandoffManifest(manifestPath).supersession, supersession);
			writeParallelHandoffGroup({
				manifestPath,
				runId: "lane-history",
				mode: "single",
				...
				diffs: [diff(dir, 0, "worker", false)],
				cleanup: cleanup("partial"),
				results: [{ agent: "worker", status: "completed", summary: "updated" }],
			});
			const updated = JSON.parse(fs.readFileSync(manifestPath, "utf-8")) as ParallelHandoffManifest;
			assert.deepEqual(updated.merge, merge);
			assert.deepEqual(updated.supersession, supersession);
		} finally {
			fs.rmSync(dir, { recursive: true, force: true });
		}
	});
