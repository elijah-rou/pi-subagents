...
import { EXTERNAL_RUN_REGISTRY_KEY, EXTERNAL_RUN_REGISTRY_VERSION, registerExternalRun } from "../../src/shared/external-runs.ts";
...
describe("native subagent fleet", () => {
	...
	it("shows cached external jobs as display-only without reading paths or invoking controls", async () => {
		...
		try {
			...
			assert.doesNotMatch(initial, /steer ·|stop ·/);
			component.handleInput("H");
			assert.match(component.render(120).join("\n"), /owning extension controls execution/);
	...
	it("uses custom Fleet keybindings without applying them inside modal text input", () => {
		...
		try {
			...
			try {
				...
				assert.match(selectedLine(), /newer/);
				component.handleInput("H");
				assert.match(selectedLine(), /newer/, "retired inspect binding must remain inert");
				assert.doesNotMatch(component.render(100).join("\n"), /action pending|Herdr/i);
