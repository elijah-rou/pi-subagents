...
async function main() {
	...
	fs.writeFileSync(callTempPath, JSON.stringify({
		...
		requiredChildTools: readRequiredToolsEnv(),
		parentSessionId: process.env.PI_SUBAGENT_PARENT_SESSION,
		permissionPolicy: process.env.PI_SUBAGENT_PERMISSION_POLICY,
	}), "utf-8");
