...
Set `subagents.directResultDefault` to `role` or `text` in Pi settings. The package-compatible default is `text`. This default applies only to direct calls; handwritten `workflowScript` children remain text unless they explicitly set `outputSchema`. External CLI/job custom agents do not support package role contracts or custom structured output and must use `resultContract: "text"`.
