...
Legacy top-level `chain`, `tasks`, `parallel`, and `chainDir` inputs; `/chain`, `/parallel`, `/run-chain`, and `/chain-prompts`; durable `.chain.md` execution; and the `append-step`, `approve-checkpoint`, and `reject-checkpoint` controls are removed. Use direct `{ agent, task }` for one child. Use `workflowScript` with `runs.run(...)`, `runs.all(...)`, and ordinary JavaScript for sequence and parallelism. Saved `.chain.md` and `.chain.json` definitions remain passive discovery inputs for one published release, but core no longer authors or executes them. Run `subagent({ action: "doctor" })` or `/subagents-doctor` to list readable definitions and diagnostics before migrating them. Existing `append-requests/*.json` files are inert, remain untouched, and are never consumed.
