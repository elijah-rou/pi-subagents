...
Direct `{agent,task}` execution accepts `resultContract:"role"|"text"`. `role` selects the bounded package-owned contract for bundled roles and the generic contract for native custom agents; `text` preserves prose. `outputSchema` is the caller-owned structured escape and cannot be combined with `resultContract`. External CLI/job custom agents support neither package role contracts nor custom structured output and must use `resultContract:"text"`. `subagents.directResultDefault` chooses the omitted behavior and defaults to `text` for compatibility. These defaults never apply to handwritten `workflowScript` children.
