...
| `timeoutMs` / `maxRuntimeMs` | number | 30 min foreground; none async | Optional run-level max runtime in milliseconds. Foreground uses 30 minutes when omitted. Async runs have no default timeout, including async workflows. Per-agent `maxTimeoutMs` clamps explicit values; the builtin researcher cannot exceed 900000ms. |
| `checkpointAfterMs` | number | per-agent or none | Requests one bounded wrap-up at the first safe point after this elapsed duration. It must be less than the effective timeout. An explicit timeout without an explicit checkpoint suppresses the agent default checkpoint. The builtin researcher default is 480000ms. |
